...
const SchedUI = {
  ...
  Init: function (overrideCache) {
    ...
    let hashObj = SchedUI.GetUrlHash();
    SchedUI.SetupPrototypes();
    // SchedUI.Options.Start = moment(SchedUI.Options.Start);

    SchedUI.Options.Start = moment(
      hashObj['start'] || SchedUI.Options.Start || moment().startOf('day')
    );
    SchedUI.Options.SelectedPeriod =
      hashObj['period'] || SchedUI.Options.SelectedPeriod;
  ...
  SetUrlHash: function (key, value) {
    ...
    for (let prop in hashObj) {
      if (hashObj.hasOwnProperty(prop)) {
        hashArr.push(
          encodeURIComponent(prop) + '=' + encodeURIComponent(hashObj[prop])
        );
      }
    }
  ...
  GetSelectedPeriod: function () {
    let period;
    for (let i = 0; i < SchedUI.Options.Periods.length; i++) {
  ...
  AddHeaderClasses: function (td, columnCount, specificHeader) {
    ...
    let tdArray, foundTD;
    // tslint:disable-next-line:one-variable-per-declaration
    let prevIndex, nextIndex, colspan;
    // tslint:disable-next-line:one-variable-per-declaration
    let complete, isEven;
  ...
  CreateCalendar: function () {
    ...
    let tr, td, header;
    // tslint:disable-next-line:one-variable-per-declaration
    let minuteDiff, splits, period, end;
    // tslint:disable-next-line:one-variable-per-declaration
    let thisTime, prevDate, fThisTime, fPrevDate, colspan;
    ...
    for (
      ...
      for (let i = 0; i < splits; i++) {
        thisTime = moment(SchedUI.Options.Start)['tsAdd'](
          'minutes',
  ...
  CreateSections: function (sections) {
    // tslint:disable-next-line:one-variable-per-declaration
    let timeCount, tr, td, sectionContainer, headers, i;
    timeCount = 1;
    ...
    for (i = 0; i < sections.length; i++) {
      ...
      td = $(document.createElement('td'))
        ...
        .appendTo(tr);
      for (let time = 0; time < timeCount; time++) {
        td = $(document.createElement('td'))
      ...
      if (SchedUI.Options.AppendWeekDaysClasses) {
        $(
          '.time-sch-content-header-wrap > table tr.time-sch-times-header-1 > td'
        ).each(function (i, d) {
          let tdClass = $(d).attr('class');
          let weekDayIndx = tdClass.indexOf('week-day-');
          if (weekDayIndx !== -1) {
            $(
              '.time-sch-content-wrap > table tr > td:nth-child(' +
                (i + 1) +
                ')'
  ...
  ShowCurrentTime: function () {
    // tslint:disable-next-line:one-variable-per-declaration
    let currentTime, currentTimeElem, minuteDiff, currentDiff, end;
    // Stop any other timeouts happening
    if (SchedUI.ShowCurrentTimeHandle) {
      clearTimeout(SchedUI.ShowCurrentTimeHandle);
    }
    currentTime = moment();
    end = SchedUI.GetEndOfPeriod(
      SchedUI.Options.Start,
      SchedUI.GetSelectedPeriod()
    );
    minuteDiff = Math.abs(SchedUI.Options.Start.diff(end, 'minutes'));
    ...
    for (i = 0; i < items.length; i++) {
      ...
      if (section) {
        ...
        if (item.start <= end && item.end >= SchedUI.Options.Start) {
          foundPos = null;
          let schedStart = SchedUI.Options.Start.milliseconds();
          ...
          if (item.events) {
            for (let ev = 0; ev < item.events.length; ev++) {
              ...
              $(document.createElement('div'))
                ...
                .attr(
                  'title',
                  event.at.format(SchedUI.Options.LowerFormat) +
                    ' - ' +
                    event.label
                )
                .data('event', event)
          ...
          for (let pos = 0; pos < inSection[item.sectionID].length; pos++) {
  ...
  SetupItemEvents: function (itemElem) {
      ...
      itemElem.draggable({
        ...
        drag: function (event, ui) {
          ...
          if (SchedUI.Options.Events.ItemMovement) {
            ...
            minuteDiff = Math.abs(
              SchedUI.Options.Start.diff(periodEnd, 'minutes')
            );
            item = $(event.target).data('item');
            start = moment(SchedUI.Options.Start)['tsAdd'](
              'minutes',
              minuteDiff *
                (ui.helper.position().left / SchedUI.SectionWrap.width())
            );
            ...
            if (item.start < SchedUI.Options.Start) {
              start.tsAdd(
                'minutes',
                item.start.diff(SchedUI.Options.Start, 'minutes')
              );
              end.tsAdd(
                'minutes',
                item.start.diff(SchedUI.Options.Start, 'minutes')
      ...
      $('.time-sch-section-container').droppable({
        ...
        drop: function (event, ui) {
          ...
          if (SchedUI.Options.Events.ItemDropped) {
            ...
            setTimeout(function () {
              SchedUI.Options.Events.ItemDropped.call(
                this,
                item,
                sectionID,
                start,
            ...
            if (SchedUI.Options.Events.ItemMovement) {
              ...
              if (ui.position.left !== ui.originalPosition.left) {
                ...
                start = moment(SchedUI.Options.Start)['tsAdd'](
                  'minutes',
                  minuteDiff *
                    ($(this).position().left / SchedUI.SectionWrap.width())
                );
                end = item.end;
              } else {
                // Right handle moved
                start = item.start;
                end = moment(SchedUI.Options.Start)['tsAdd'](
                  'minutes',
                  minuteDiff *
                    (($(this).position().left + $(this).width()) /
                      SchedUI.SectionWrap.width())
                );
              }
              SchedUI.Options.Events.ItemMovement.call(this, item, start, end);
            }
    ...
    if (SchedUI.Options.Events.ItemEventClicked) {
      itemElem.find('.time-sch-item-event').click(function (event) {
        ...
        event.preventDefault();
        SchedUI.Options.Events.ItemEventClicked.call(
          this,
          $(this).data('event'),
          itemElem.data('item')
        );
    ...
    if (SchedUI.Options.Events.ItemEventMouseEnter) {
      itemElem.find('.time-sch-item-event').mouseenter(function (event) {
        ...
        SchedUI.Options.Events.ItemEventMouseEnter.call(
          this,
          $(this).data('event'),
          itemElem.data('item')
        );
    ...
    if (SchedUI.Options.Events.ItemEventMouseLeave) {
      itemElem.find('.time-sch-item-event').mouseleave(function (event) {
        ...
        event.preventDefault();
        // SchedUI.Options.Events.ItemEventClicked.call(this, $(this).data('event'), itemElem.data('item'));
        SchedUI.Options.Events.ItemEventMouseLeave.call(
          this,
          $(this).data('event'),
          itemElem.data('item')
        );
  ...
  FillSections_Callback: function (obj) {
    ...
    if (
      $('.time-sch-content-wrap > .time-sch-table-content > tbody > tr')
        .length === 0
    ) {
      $('.time-sch-content-wrap').after(
        '<div class="no-elements">' +
          SchedUI.Options.Text.NoElementsToDisplay +
          '</div>'
      );
    } else {
      $('.no-elements').remove();
  ...
  FillSchedule: function () {
    ...
    SchedUI.Options.GetSchedule.call(
      this,
      SchedUI.FillSchedule_Callback,
      SchedUI.Options.Start,
      end
    );
  },
  FillSchedule_Callback: function (obj) {
    ...
    $('.time-sch-content-wrap > table > tbody > tr').each(function (i, d) {
      $(
        'div.time-sch-section-wrapper > div.time-sch-section-container:nth-child(' +
          (i + 1) +
          ')'
  ...
  FillHeader: function () {
    ...
    SchedUI.HeaderWrap.empty().append(periodContainer, timeContainer, title);
    ...
    title.text(
      SchedUI.Options.Start.format(SchedUI.Options.HeaderFormat) +
        ' - ' +
        end.tsAdd('minutes', -1).format(SchedUI.Options.HeaderFormat)
    );
    for (let i = 0; i < SchedUI.Options.Periods.length; i++) {
      period = SchedUI.Options.Periods[i];
